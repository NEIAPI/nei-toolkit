...
下面是 `outputModel` 字段值的计算规则:

```html
根据返回值的类型，需返回不同的值，规则如下：
1. 如果没有定义返回值，则返回: null。
2. 如果返回值为一个导入的 `ResultData`（即只有一个可变类型字段），根据可变类型字段的类型，则返回：
    a. 如果可变类型字段为基本类型(字符、数值、布尔)，则返回基本类型的名称。
    b. 如果可变类型字段为自定义哈希类型 CustomModel，则返回 CustomModel。
    c. 如果可变类型字段是数组，则:
        i.  如果数组元素是基本类型(字符、数值、布尔) 或者自定义类型 CustomModel，则分别返回 '{{基本类型名称}}[]' 或者 'CustomModel[]'。
        ii. 如果数组元素是数组(二维数组)，则根据二维数组的元素类型，递归执行规则 c。例如，如果二维数组的元素是 String, 则返回 String[][]。
