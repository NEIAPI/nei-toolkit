...
module.exports = {
     ...
     *    g. 如果可变类型字段为自定义哈希类型 CustomModel，则返回 {typeName: `CustomModel`}。
     *    h. 如果可变类型字段是数组，则:
    ...
    getOutputModel(params) {
        ...
        let getArrayElement = (datatype) => {
            let arrDimLen = 0;
            let getArrayElementType = (dt) => {
                if (dt.isArray) {
                    arrDimLen++;
                    let arrDatatype = this.neids.datatypes.find((item) => {
                        return item.id === dt.type;
                    });
                    return getArrayElementType(arrDatatype);
                } else if (dt.format === neiDbConst.MDL_FMT_ARRAY) {
                    // 数组类型
                    arrDimLen++;
                    let arrDatatype = dt.params[0];
                    if (arrDatatype.isArray) {
                        return getArrayElementType(arrDatatype);
                    } else {
                        return arrDatatype.typeName;
                    }
                } else {
                    return dt.name;
                }
            }
            let arrElementTypeName = getArrayElementType(datatype);
            return {
                typeName: arrElementTypeName,
                dim: arrDimLen
            }
        }
        if (datatypeName) {
            ...
            if (variableTypes.length === 1) {
                ...
                if (varType.isArray) {
                    return getArrayElement(varType);
                } else {
                    return {
