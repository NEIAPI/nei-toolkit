...
module.exports = {
    ...
    format(neids) {
        ...
        ds.interfaces = neids.interfaces.map((item) => {
            ...
            let reqSplitHeader = getSplitReqHeaders(nItem.reqHeaders);
            nItem.reqConstHeaders = reqSplitHeader.constHeaders;
            nItem.reqVarHeaders = reqSplitHeader.varHeaders;
        ...
        neids.datatypes.forEach((item) => {
            if (item.type === neiDbConst.MDL_FMT_HASH
                ...
                }).forEach((item)=>{depModels[item.name] = item.type});
                if (!foundVar) {
