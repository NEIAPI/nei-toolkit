...
module.exports = {
    ...
    format(neiDs) {
        ...
        ds.interfaces = neiDs.interfaces.map((item) => {
            ...
            var nItem = {
                ...
                inputs: null,
                outputs: null,
                importedOutput: this.getOutputModel(item.params.outputs),
                reqHeaders: null,
     ...
     *    a. 如果可变类型字段为基本类型(字符、数值、布尔)，则返回基本类型的名称。
     *    g. 如果可变类型字段为自定义哈希类型 CustomModel，则返回 CustomModel。
     *    h. 如果可变类型字段是数组，则:
     *      i.  如果数组元素是基本类型(字符、数值、布尔) 或者自定义类型 CustomModel，则分别返回 '{{基本类型名称}}[]' 或者 'CustomModel[]'。
     *      ii. 如果数组元素是数组(二维数组)，则根据二维数组的元素类型，递归执行规则 h。例如，如果二维数组的元素是 String, 则返回 String[][]。
     * 3. 如果返回值为一个导入的自定义类型 CustomModel（但不是 ResultData），则返回：CustomModel。
     * 4. 如果返回值只有一个字段，则根据它的类型，按规则 2 执行。
     * 5. 如果返回值的字段个数大于 1, 则返回 `Object`。
     * 6. 其他情况返回：null。
     *
     * @param  {object} params - 参数数组
     * @return  {string|null} 输出参数的类型
     */
    getOutputModel(params) {
        ...
        if (datatypeName) {
            // 是一个导入类型
            // 先验证是否只有一个可变类型
            let variableTypes = params.find((item) => {
