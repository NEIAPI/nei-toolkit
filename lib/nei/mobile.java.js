...
pro._exportModelJava = function (config, options, data) {
    var root = config.resOut || util.format('%s%s/Models/', config.proRoot, config.id);
...
pro._exportRequestJava = function (config, options, data) {
    var self = this;
    var root = config.resOut || util.format('%s%s/HttpTasks/', config.proRoot, config.id);
...
pro._build = function (config, options, data) {
    ...
    switch (config.projectType) {
        case 'hz.netease':
            config.resOut = config.proRoot + (config.resOut || 'app/src/main/hthttp-gen/');
