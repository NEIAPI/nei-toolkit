 ...
 * @return {Object} prop
 */
pro._getPropJava = function (attr, pkgs) {
    var conf = this._dataTypeMapJava[attr.type];
    if (!conf) {
        var it = this.dataTypes[attr.type];
        conf = {
            typeName: it.name
        };
        // check enum type
...
pro._formatPropJava = function (type, conf) {
    ...
    if (!type.props) {
        ...
        type.attrs.forEach(function (attr) {
            type.props.push(this._getPropJava(attr, type.pkgs));
        }, this);
    }
    return type;
};
...
pro._exportModelJava = function (config, options, data) {
    ...
    ret.models.forEach(function (id) {
        ...
        var type = this._formatPropJava(this.dataTypes[id], modConfig);
        var file = getFileName(type.name);
        var content = this._mergeTemplate(
...
pro._exportRequestJava = function (config, options, data) {
    ...
    var getParams = function (itf) {
        ...
        if (self._isAnImportedDataType(itf.inputs)) {
            var input = self.dataTypes[itf.inputs[0].datatypeId];
...
pro.model = function (config, options, data) {
    ...
    function exec(config, options, data) {
        config.resOut = _path.normalize(config.proRoot + (config.resOut || '/app/src/main/hthttp-gen/') + '/');
        self.emit('debug', {
            data: [config.resOut],
...
pro._buildHZNetEaseProject = function (config) {
    ...
    var templates = {};
    var files = _fs.lsfile(config.proRoot, (fileName, file) => {
