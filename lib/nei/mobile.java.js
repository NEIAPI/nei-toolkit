...
class MobileJavaBuilder extends MobileBuilder {
    ...
    getPropJava(attr, pkgs) {
        ...
        if (prop.isArray == 1) {
            ...
            // 这里按需求方的要求, 如果是一维数组, 为了兼容现在已经在使用的项目, 仍旧使用 List
            // 如果数组大于一维, 则使用 `String[][]` 类似的形式
    ...
    buildHZNetEaseProject() {
        ...
        if (config.templateDataPath) {
            tplData = Object.assign({}, _util.file2json(config.templateDataPath, true), config.templateData);
        }
