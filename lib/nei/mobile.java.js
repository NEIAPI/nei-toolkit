...
var TEMPLATE_PKG_ROOT = EMPTY_PROJECT_ROOT + 'app/src/main/java/com/netease/template/';
...
pro._build = function (config, options, data) {
    ...
    this.emit('debug', {
        message: 'copying project files succeed'
    });
    var params = {
        "packageName": "com.alibaba.template",
        "shortPackageName": "NEI",
        "applicationClass": "MyApplication"
    };
    if (params.packageName !== 'com.netease.template') {
        // copy template files
        var templatePkgPath = util.format(
            '%sapp/src/main/java/%s/',
            config.proRoot,
            params.packageName.replace(/\./g, '/')
        );
        this.emit('debug', {
            data: [templatePkgPath],
            message: 'making template package dir: %s'
        });
        _fs.mkdir(templatePkgPath);
        this.emit('debug', {
            data: [TEMPLATE_PKG_ROOT, templatePkgPath],
            message: 'copying template files: from "%s" to "%s"'
        });
        _fs.cpdir(TEMPLATE_PKG_ROOT, templatePkgPath, null);
        this.emit('debug', {
            message: 'copying template files succeed'
        });
        this.emit('debug', {
            data: [TEMPLATE_PKG_ROOT],
            message: 'removing old template dir: %s'
        });
        _fs.rmdir(TEMPLATE_PKG_ROOT);
        this.emit('debug', {
            message: 'removing old template dir succeed'
        });
    }
    var templates = {};
