...
class MobileJavaBuilder extends MobileBuilder {
    ...
    exportModelJava() {
        ...
        //继承baseModel,added by Icy
        let req = {
            baseModel: null,
            baseModelImport: null
        };

        let importBaseModelPkg = (req) => {
            // 如果 BaseModel 和生成的请求在同一个包内，不需要import
            let modelPkg = `${modConfig.aPkg}.${modConfig.htPkg}`;
            let baseModelPkgPaths = modConfig.baseModel.split('.');
            baseModelPkgPaths.pop();
            if (baseModelPkgPaths.join('.') !== modelPkg) {
                req.baseModelImport = modConfig.baseModel;
            }
        }
        
        if (modConfig.baseModel) {
            req.baseModel = (function () {
                let paths = modConfig.baseModel.split('.');
                return paths[paths.length - 1];
            })();
            
            importBaseModelPkg(req)
        }
    ...
    exportRequestJava() {
        ...
        let getParams = (itf) => {
            ...
            let getParamValueExp = (param) => {
                if (param.isArray) {
                    ...
                    return param.name;//根据需求,删除toString by Icy
                    ...
                    if (param.typeName.indexOf('[]') > -1) {
                        ...
                        return `${param.varName || param.name}`;//根据需求,删除toString by Icy
                    }
                    // 自定义类型为哈希表
        ...
        this.data.interfaces.forEach((itf) => {
            pkgs = {
                'com.netease.hearttouch.hthttp2.operation.HttpMethod': 1 // 固定要导入的包
            };
            importBaseReqPkg();
            let method = this.getReqMethod(itf.method);
