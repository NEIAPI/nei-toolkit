...
var EMPTY_PROJECT_ROOT = __dirname + '/mobile/java/';
var TEMPLATE_PKG_PATH = 'app/src/main/java/com/netease/template/';
...
pro._exportModelJava = function (config, options, data) {
    ...
    var getFileName = function (typeName) {
        return util.format(
            '%s/%s.java',
            (root + conf.aPkg + '/' + conf.mPkg).replace(/\./g, '/'),
            typeName
        );
    };
    // build enum type
...
pro._exportRequestJava = function (config, options, data) {
    var self = this;
    var root = util.format('%s%s/HttpTasks/', config.proRoot, config.id);
...
pro.model = function (config) {
    ...
    var self = this;
    self.initConfig(config, function (config, options, data) {
        self._sortData(data);
 ...
 * @param  {String}  config.packageName - application's template package's new package name
 * @param  {String}  config.shortPackageName - application's short package name
 * @param  {String}  config.applicationClass - application's entry class name
...
pro._build = function (config, options, data) {
    this.emit('debug', {
        data: [EMPTY_PROJECT_ROOT, config.proRoot],
        message: 'copying project files: from "%s" to "%s"'
    });
    _fs.cpdir(EMPTY_PROJECT_ROOT, config.proRoot, null);
    var tplPkgPath = config.proRoot + TEMPLATE_PKG_PATH;
    if (config.packageName !== 'com.netease.template') {
        // copy template files
        ...
        this.emit('debug', {
            data: [tplPkgPath],
            message: 'making template package dir: %s'
        });
        _fs.mkdir(tplPkgPath);

        var oldTplPkgPath = config.proRoot + TEMPLATE_PKG_PATH;
        this.emit('debug', {
            data: [oldTplPkgPath, tplPkgPath],
            message: 'copying template files: from "%s" to "%s"'
        });
        _fs.cpdir(oldTplPkgPath, tplPkgPath, null);

        this.emit('debug', {
            data: [oldTplPkgPath],
            message: 'removing old template dir: %s'
        });
        _fs.rmdir(oldTplPkgPath);
    }
    if (config.applicationClass !== 'YXApplication') {
        // rename entry class file
        var applicationEntryClass = tplPkgPath + 'application/YXApplication.java';
        var newApplicationEntryClass = util.format(tplPkgPath + 'application/%s.java', config.applicationClass);
        this.emit('debug', {
            data: [config.applicationClass],
    ...
    var files = _fs.lsfile(config.proRoot, function (fileName, file) {
        if (/^\.(java|gradle|xml)$/.test(path.extname(fileName))) {
            ...
            try {
                ...
                this.emit('error', {
                    data: [err],
                    message: 'complie template error: %s'
                });
            }
            return false;
        }
    ...
    var fillData = {
        ...
        shortPackageName: config.shortPackageName,
