...
pro._exportRequestJava = function (config, options, data) {
    ...
    var baseReqClass = (function () {
        if (modConfig.base.indexOf('.') !== -1) {
            ...
            baseReqPkg = 'com.netease.hthttp' + '.' + modConfig.base
        }
...
pro._build = function (config, options, data) {
    ...
    switch (config.projectType) {
        case 'hz.netease':
            ...
            this.emit('debug', {
                message: "Start building models and requests..."
            });
 ...
 * @param  {Object}  config - config object, see @pro.__build
 * @return {Void}
 */
pro._buildHZNetEaseProject = function (config) {
    ...
    if (tplData.packageName !== 'com.netease.template') {
        // copy template files
        tplPkgPath = util.format(
            '%sapp/src/main/java/%s/',
            config.proRoot,
            tplData.packageName.replace(/\./g, '/')
        );
        this.emit('debug', {
            data: [tplPkgPath],
