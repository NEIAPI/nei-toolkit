...
pro.model = function (config, options, data) {
    ...
    function exec(config, options, data) {
        config.resOut = config.proRoot + (config.resOut || (config.productName + '/Network/'));
        self.emit('debug', {
            data: [config.resOut],
        ...
        self._checkData(data, config, function (data) {
            ...
            if (config.action === 'build' || self.isResourceAdded || self.isResourceDeleted) {
    ...
    switch (config.projectType) {
        case 'hz.netease':
            ...
            if (config.templateDataPath) {
                var templateDataPath =_path.absolute(
                    config.templateDataPath, process.cwd()+'/'
                );
                tplData = _util.file2json(templateDataPath);
            }
...
pro._buildHZNetEaseProject = function (config) {
    ...
    var templatePath =_path.absolute(
        config.templatePath, process.cwd()+'/'
    );
    this.emit('debug', {
