...
pro._exportModelOC = function (config, options, data) {
    ...
    ret.models.forEach(
        function (id) {
            ...
            // check type
            var type = this._formatPropOC(this.dataTypes[id], conf);
...
pro._exportRequestOC = function (config, options, data) {
    var self = this;
    var root = config.resOut + 'Requests/';
    this.emit('debug', {
        data: [root],
    ...
    var getReqInfo = function (inputs) {
        ...
        var setDataTypeAndModel = function (input, dataTypeId) {
            var type = self.dataTypes[dataTypeId];
            type.klses = {};
...
pro._build = function (config, options, data) {
    switch (config.projectType) {
        case 'hz.netease':
            var tplData = _util.file2json(config.templateDataPath, true);
