...
class MobileOCBuilder extends MobileBuilder {
    ...
    modelStart() {
        ...
        let config = this.config;
        this.initModConfig();
        let commandLineArgs = this.config[Object.getOwnPropertySymbols(this.config)[0]].args;
        // 命令行参数优先级最高
        if (commandLineArgs.resOut) {
            config.resOut = _path.normalize(`${config.outputRoot}/${config.resOut}/`);
        } else {
            // mobile 命令不需要 productName 参数
            config.resOut = _path.normalize(`${config.outputRoot}/${config.productName || ''}/${config.resOut}/`);
        }
        logger.log('debug', {
            data: [config.resOut],
        ...
        if (config.action === 'build'
            ...
            // 查找当前目录下面的 xxxx.xcodeproj/project.pbxproj 文件
            let fileList = fs.readdirSync(config.outputRoot);
            ...
            let projectPath = _path.normalize(config.outputRoot + productName);
    ...
    buildStart(empty) {
        switch (this.config.projectType) {
            case 'hz.netease':
                let savedData = this.config[Object.getOwnPropertySymbols(this.config)[0]]
                this.tplData = Object.assign(
                    {},
                    _util.file2json(this.config.templateDataPath, true),
                    savedData.templateData,
                    {
                        ProductName: this.config.productName,
                        Prefix: this.config.namePrefix, // issue 18
                        CategoryPrefix: this.config.namePrefix.toLowerCase() // issue 18
                    }
                );
    ...
    buildHZNetEaseProject() {
        ...
        let savedData = this.config[Object.getOwnPropertySymbols(this.config)[0]];
        // 如果在命令行中指定了输出参数
        let commandLineOutput = savedData.args.p || savedData.args.project;
        if (commandLineOutput) {
            config.outputRoot = _path.absolute(`${commandLineOutput}/`, savedData.configFileData.dir);
        } else {
            config.outputRoot = _path.absolute(`${config.project}/${config.productName}/`, savedData.configFileData.dir);
        }
