...
pro._exportModelOC = function (config, options, data) {
    var root;
    if (config.modelOutPath) {
        root = config.modelOutPath + '/Models/';
    } else {
...
pro._exportRequestOC = function (config, options, data) {
    ...
    var root;
    if (config.modelOutPath) {
        root = config.modelOutPath + '/Requests/';
    } else {
...
pro._build = function (config, options, data) {
    ...
    switch (config.projectType) {
        case 'hz.netease':
            ...
            config.modelOutPath = config.proRoot + tplData.ProductName + '/Network';
            this.emit('debug', {
                data: [config.modelOutPath],
                message: "make models and requests' dir: %s"
            });
            _fs.mkdir(config.modelOutPath);
            ...
            //if (this.isResourceAdded || this.isResourceDeleted) {
            // update project.pbxproj file
            _pbx.update(config.tplData.ProductName, config.proRoot + config.tplData.ProductName);
            //}
            break;
        default:
            break;
...
pro._buildHZNetEaseProject = function (config) {
    ...
    var productFolderPath = config.proRoot + config.tplData.ProductName + '/';
    this.emit('debug', {
    ...
    var xcodeprojFolderPath = config.proRoot + config.tplData.ProductName + '.xcodeproj/';
    this.emit('debug', {
    ...
    _fs.walk(
        ...
        function (filePath) {
            ...
            try {
                ...
                // 如果 tokens.length === 1, 则说明没有需要替换的内容
    ...
    files.forEach(function (file) {
        ...
        if (!!func) {
            ...
            if (/^\.(pbxproj)$/.test(path.extname(file))) {
                // 这个文件之后马上需要读取并更新它的内容，所以这里需要使用同步输出方法
