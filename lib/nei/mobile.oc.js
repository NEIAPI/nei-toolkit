...
pro._exportRequestOC = function (config, options, data) {
    var self = this;
    var root = config.modelOutPath || util.format('%s%s/Requests/', config.proRoot, config.id);
    var conf = this._modConfig || {};
...
pro._build = function (config, options, data) {
    ...
    switch (config.projectType) {
        case 'hz.netease':
            ...
            _fs.mkdir(config.proRoot + '/Network');
            config.modelOutPath = config.proRoot + '/Network/';
            this.emit('debug', {
