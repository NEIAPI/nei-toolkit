 ...
 * @return {Object} prop
 */
pro.__getProp = function (type, attr) {
    var conf = this.__dataTypeMap[attr.type];
    if (!conf){
 ...
 * @return {Object} type
 */
pro.__formatProp = function (type, conf) {
    // check properties
    if (!type.props){
        type.props = [];
        type.klses = {};
        ...
        type.attrs.forEach(
            function(attr){
                type.props.push(this.__getProp(type, attr));
            },this
        );
        // for dictStr
        var arr = [];
...
pro._exportModelOC = (function(){
    ...
    return function(config,options,data){
        ...
        ret.model.forEach(
            function(id){
                // ignore system type
                if (!!this.__dataTypeMap[id]){
                    return;
                }
                // check type
                var type = this.__formatProp(this._types[id],conf),
...
pro._exportRequestOC = function(config,options,data){
    ...
    var getReqInfo = function (inputs) {
        ...
        inputs.forEach(function (input) {
            var type = self._types[input.type];
            var prop = self.__getProp(type, input);
        ...
        return {
            dataTypes: dataTypes,
            models: _.uniq(models.sort(), true)
        }
    };
    var reqs = [];
    var req;
