 ...
 * @return {Array}  returnObj.diffDesc - changed description
 * @return {Object} returnObj.diffs - contain changed resources
 * @return {Array}  returnObj.diffs.datatypes - changed data type ids
 * @return {Array}  returnObj.diffs.interfaces - changed interface ids
 */
            ...
            if (cObj.type === 'array') {
                oData[key].forEach(function (data, index) {
                    ...
                    if (foundResults.length === 1) {
                        ...
                        diffDesc.push(getDescExp(parent) + '第' + (index + 1) + '个' + cObj.name + '(id: ' + data.id + ', name: "' + data.name + '")' + '已被删除');
                    }
                });
                // 再判断新增的情况
                nData[key].forEach(function (data, index) {
                    ...
                    if (foundResults.length === 0) {
                        setDiff(parent);
                        diffDesc.push(getDescExp(parent, true) + '新增了' + cObj.name + '(id: ' + data.id + ', name: "' + data.name + '")');
                    }
                });
                ...
                Object.keys(oData[key]).forEach(function (oKey) {
                    if (nData[key][oKey]) {
                        ...
                        setDiff(parent);
                        diffDesc.push(cObj.name + oKey + '已被删除');
                    }
                });
                // 再判断新增的情况
                Object.keys(nData[key]).forEach(function (oKey) {
                    if (!oData[key].hasOwnProperty([oKey])) {
                        diffDesc.push('新增了' + cObj.name + oKey);
                    }
                });
            } else {
                if (oData[key] !== nData[key]) {
                    setDiff(parent);
                    diffDesc.push(getDescExp(parent) + '"' + cObj + '(' + key + ')"' + '已更改');
                }
            }
        });
    };
    checkData(oldData, newData, this._comparedMap);
...
pro._checkData = function (newData, config, callback) {
    ...
    this._get(jsonFile, function (fileData) {
        try {
            ...
            var diffResult = self._diff(json, newData);
            var diffTxt = config.proRoot + 'JSON/nei-' + config.id + '-diff.txt';
        ...
        var file = config.proRoot + 'JSON/nei-' + config.id + '-' + time + '.json';
