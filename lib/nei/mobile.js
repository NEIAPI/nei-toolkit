...
pro._exportRequestOC = function(config,options,data){
    ...
    var getReqRes = function (outputs) {
        ...
        if (dataTypeName) {
            ...
            if (isResultDataModel) {
                resultOutput = _.find(outputs, function (output) {
    ...
    var getReqInfo = function (inputs) {
        ...
        var models = [];
        var autoAddedName;
        var isAnImportedDataType = function () {
            var datatypeId;
            for(var i = 0, l = inputs.length; i < l; i++) {
                if (!datatypeId) {
                    datatypeId = inputs[i].datatypeId
                }
                if (datatypeId !== inputs[i].datatypeId) {
                    return false;
                }
            }
            if (datatypeId) {
