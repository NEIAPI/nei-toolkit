...
class NodeBuilder extends Builder {
    ...
    updateStart() {
        ...
        if (this.config.view) {
            this.buildTemplates();
            logger.log('debug', {
                message: 'views are updated'
            });
        } else {
            logger.log('debug', {
                message: 'views are not updated'
            });
        }
    ...
    buildHZNetEaseProject() {
        ...
        files.forEach((file) => {
            ...
            if (func) {
                ...
                }, this.config[Object.getOwnPropertySymbols(this.config)[0]], this.config));
    ...
    buildTemplatePage(list, commonFilePath) {
        ...
        (list || []).forEach((it) => {
            ...
            let templateData = Object.assign({}, config[Object.getOwnPropertySymbols(config)[0]], {
    ...
    buildTemplateRules() {
        ...
        (list || []).forEach((it) => {
            ...
            tpls.forEach((id) => {
                ...
                ret.push({
                    id: id,
                    path: this.parseFileName(tpl.path, this.config.viewRoot)
                })
            });
    ...
    buildServerConfig() {
        ...
        let I_MOCK_ROOT = `${config.outputRoot}mock.data/interface/`;

        // template mock dir path
        let T_MOCK_ROOT = `${config.outputRoot}mock.data/template/`;

        // 按 rules 的 path 字段进行排序, 参数多的排前面, 使得 jtr 优先匹配
        ...
        let fmap = {
            'jtr.js.tpl': {
                ...
                WEB_ROOT: webRoot,
                VIEW_ROOT: viewRoot,
                VIEW_EXTENSION: suffix[config.engine] || 'ejs',
                VIEW_ENGINE: config.engine,
