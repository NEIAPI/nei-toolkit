...
class NodeBuilder extends Builder {
    ...
    buildTemplatePage(list, commonFilePath) {
        ...
        (list || []).forEach((it) => {
            ...
            content = this.mergeTemplate(`${this.options.tplRoot}page.css`);
            this.output(file, content);
            if (config.mcss) {
                // 页面 mcss 样式文件
                file = `${webRoot}src/page/${filename}.mcss`;
                content = this.mergeTemplate(`${this.options.tplRoot}page.mcss`);
                this.output(file, content);
            }
            // 页面脚本
            file = `${webRoot}src/page/${filename}.js`;
            content = this.mergeTemplate(`${this.options.tplRoot}page.js`);
            this.output(file, content);
        });
    }
