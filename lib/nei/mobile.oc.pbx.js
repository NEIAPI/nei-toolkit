...
class PbxProj {
  ...
  addFile(fileName, groupKey) {
    ...
    if (extName === '.h') {
      ...
      this.myProj.removeFile(fileName, groupKey, {defaultEncoding: 4});
      this.myProj.addFile(fileName, groupKey, {defaultEncoding: 4});
    } else {
      this.myProj.removeFile(fileName, groupKey);
