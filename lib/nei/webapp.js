...
class WebAppBuilder extends Builder {
    ...
    buildServerConfig() {
        ...
        if (config.tMockRoot) {
            ...
            T_MOCK_ROOT = viewRoot + 'src/mock/';
        }

        // engine file path
        let ENGINE_ROOT;
        if (config.engineRoot) {
            ENGINE_ROOT = _path.normalize(
                path.relative(file, config.engineRoot) + '/'
            );
        } else {
            ENGINE_ROOT = webRoot + 'src/javascript/lib/express-freemarker/index.js';
        }
