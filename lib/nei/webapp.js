...
class WebAppBuilder extends Builder {
    ...
    buildServerConfig() {
        ...
        // config.webRoot 和 config.viewRoot 已经是绝对地址了
        ...
        let I_MOCK_ROOT;
        if (config.iMockRoot) {
            if (path.isAbsolute(config.iMockRoot)) {
                I_MOCK_ROOT = config.iMockRoot;
            } else {
        ...
        let T_MOCK_ROOT;
        if (config.tMockRoot) {
            if (path.isAbsolute(config.tMockRoot)) {
                T_MOCK_ROOT = config.tMockRoot;
            } else {
        ...
        let fmap = {
            'jtr.js.tpl': {
                ...
                WEB_ROOT: webRoot,
                VIEW_ROOT: viewRoot,
                VIEW_EXTENSION: suffix[config.engine] || 'ftl',
