...
let JavaWebGenerator = require('./javaWebGenerator');
...
class Builder extends EventEmitter {
                ...
                if (doc.type === neiDbConst.SPC_NOD_FILE) {
                    ...
                    switch (doc.dataSource) {
                        ...
                        case neiDbConst.SPC_DTS_INTERFACE:
                            ...
                            if((this.isJavaWebProject())){//
                                JavaWebGenerator.genFiles(this.ds.interfaces,this.data, doc, dir,compileInVm);
                                break;
                            };
    ...
    buildServerConfig() {
        ...
        if (this.config.action === 'update') {
            // 更新的时候，launch、port、online、apiResHeaders 参数需要保留，因为这些参数非常可能是用户仍旧希望保留的
            let existNeiServerConfig = _util.file2json(neiServerConfigFilePath);
            ['launch', 'port', 'online', 'apiResHeaders'].forEach((property) => {
     ...
     */
    isJavaWebProject(){
        return this.options.args.specType === 'web';
    }
    ...
    output(file, content, ignoreCheck) {
        let overWrite = this.args&&this.args.overwrite;
