...
pro._parseConfig = function(next,content){
    ...
    try{
        ...
        // interfaces   - [{id,name,description,path,method,isRest,inputs,outputs}]
            ...
            if (conf.format==1){
                ...
                attrs.forEach(function(it){
                    ...
                    if (!test[it.type]){
                        ...
                        console.log('circular reference %s:%s',it.type,it.name);
                    }
                    ret[it.name] = dat;
                },this);
            }
 ...
 * @param  {Boolean} config.fiddler - whether generator fiddler config
 * @return {Void}
 */
...
pro._mockInterface = function(config,options,data){
    ...
    // output fidder config
    if (!this._mockFiddler){
        return;
    }
    // generator fiddler rules
    var rules = [];
    list.forEach(function(it){
        rules.push({
            path:it.path,
...
pro.export = (function(){
    ...
    return function(config){
        ...
        // check overwrite
        if (_fs.exist(this.__expFile)&&
            !this._options.overwrite){
            this.emit('error',{
                data:[this.__expFile],
