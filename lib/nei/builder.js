...
class Builder extends EventEmitter {
     ...
     */
    getReqMethod(value, name) {
        // 针对有范云协作项目作下特殊处理, 之后再删除
        if (name && this.config.id === "10971" && name.indexOf('PATCH') > -1) {
            return 'PATCH';
        }
    ...
    getInterfaceRequestUrl(it) {
        let method = this.getReqMethod(it.method, it.name);
        let url = it.path;
    ...
    buildInterfaceMock(list, config) {
        ...
        (list || []).forEach((it)=> {
            let method = `./${this.getReqMethod(it.method, it.name).toLowerCase()}/`;
    ...
    buildInterfaceRules(list) {
        (list || []).forEach((it) => {
            ...
            this.rules.push({
                ...
                method: this.getReqMethod(it.method, it.name),
                id: it.id,
                type: 'api',
                name: it.name
            });
    ...
    start(next, data) {
        // save update time
        if(!data){
            this.config.updateTime = (new Date()).toUTCString();
        }
        else {
            ...
            Object.keys(cMap).forEach((key) => {
                ...
                if (cObj.type === 'array') {
                    ...
                    if (oData[key] !== nData[key]) {
                        ...
                        //如果同一个id 的interface className 不同 ,如果使用了 overWrite 参数就删除以前的
                        //by 
                        if (key === "className") {

                            let getFileName = (typeName) => {
                                let filePath = (this.config.resOut + this.modConfig.aPkg + '/' + this.modConfig.htPkg).replace(/\./g, '/');
                            ...
                            let oldReqClassName = getReqClassName(oData);
                            let fileName = getFileName(oldReqClassName);

                            if (this.config.doNotOverwrite !== true) {
                                _fs.rm(fileName);
                                logger.log('debug', {
