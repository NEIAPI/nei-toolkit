...
class Builder extends EventEmitter {
    ...
    constructor(config, options) {
        ...
        this.cnfRoot = `${config.outRoot}nei.${config.id}/`;
        this.templates = {};
     ...
     */
    build(data) {
        this.startAction(this.buildStart, data);
    }
     ...
     */
    update() {
        this.startAction(this.updateStart, data);
    }
    ...
    mock(config) {
        ...
        this.startAction(this.mockStart, data);
    }
    ...
    mockTemplate() {
        ...
        this.buildTemplateMock(list, {
            mockRoot: this.config.outRoot + sufix,
    ...
    mockInterface() {
        ...
        // check suffix
        let mockRoot = this.config.outRoot;
        if (this._mockType === 0) {
     ...
     */
    mockDataType() {
        let mockRoot = this.config.outRoot + 'meta/';
        Object.keys(this.dataTypes).forEach((key)=> {
            let it = this.dataTypes[key];
            var file = mockRoot + it.name + '.json';
            var data = this.genMockData({
                type: it.id
            });
    ...
    export(config) {
        ...
        this.__expFile = `${this.config.outRoot}nei-${this.config.id}-postman.json`;
        // check overwrite
        ...
        this.startAction(this.exportStart, data);
    }
