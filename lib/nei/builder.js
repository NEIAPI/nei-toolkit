...
class Builder extends EventEmitter {
    ...
    buildServerConfig() {
        ...
        let convertToArray = function (routes) {
        ...
        if (this.config.action === 'update') {
            // 更新的时候，launch、port、online、https、apiResHeaders、customRoutes 参数需要保留，因为这些参数非常可能是用户仍旧希望保留的
            let existNeiServerConfig = _util.file2json(neiServerConfigFilePath);
            ['launch', 'port', 'online','https', 'apiResHeaders', 'customRoutes'].forEach((property) => {
                if (existNeiServerConfig.hasOwnProperty(property)) {
                    neiServerConfig[property] = existNeiServerConfig[property];
                    if(property == "customRoutes"){
                      neiServerConfig[property].toString = function(){
                          return JSON.stringify(this,null, 4)
                      }
                    }
                }
