...
class Builder extends EventEmitter {
    ...
    buildDocsTree() {
        ...
        let genFiles = (ds, itemName, doc, dir) => {
            ds.forEach((item) => {
                ...
                data[itemName] = item;
                let filename = Handlebars.compile(doc.name)(data);
                let file = _path.normalize(`${dir}/${filename}`);
                let content = Handlebars.compile(doc.content || '')(data);
                this.output(file, content);
            });
        };
        let genDocs = (docs, dir) => {
            if (this.shouldUpdateNormalDoc) {
                // 如果事先不创建文件夹, 在下载文件的时候会报错
                _fs.mkdir(dir);
            }
            docs.forEach((doc) => {
                    ...
                    switch (doc.dataSource) {
                        case neiDbConst.SPC_DTS_NONE:
                            if (this.shouldUpdateNormalDoc) {
                                ...
                                let filename = Handlebars.compile(doc.name)(this.data);
                                let file = _path.normalize(`${dir}/${filename}`);
                                let content = Handlebars.compile(doc.content)(this.data);
                                this.output(file, content);
                            }
                            break;
     ...
     */
    canGenNormalDoc() {
        return this.config.action === 'build' || (this.config.action === 'update') && this.args.spec && this.args.overwrite;
    }
