...
class Builder extends EventEmitter {
  ...
  buildServerConfig() {
    ...
    let neiServerConfig = {
      launch: this.args.launch,
      port: this.args.port,
      online: false,
      https: false,
      reload: typeof this.args.reload === 'boolean' ? this.args.reload : true,
      openUrl: '',
      apiResHeaders: null,
      watchingFiles: {
        compilers: {
    ...
    if (this.config.action === 'update') {
      ...
      Object.keys(neiServerConfig.proxyRoutes).forEach(k=>{
        let m = k.split(/\s+/);
        var rule = {
          method: m[0],
          path: m[1],
          value: neiServerConfig.proxyRoutes[k]
        };
        rules.unshift(rule);
      })
    }
