                ...
                if (doc.type === neiDbConst.SPC_NOD_FILE) {
                    ...
                    switch (doc.dataSource) {
                        ...
                        case neiDbConst.SPC_DTS_INTERFACE:
                            // 如果是iOS项目并且接口有变化, 则把已有的接口都先删除, 然后再创建这个目录
                            if (this.isIosProject() && this.diffResult.interfaceChanged && !isIOSInterfaceDirRemoved) {
                                logger.log('debug', {
                                    message: `删除接口文件夹: ${dir}/`
                                });
                                _fs.rmdir(`${dir}/`);
                        ...
                        case neiDbConst.SPC_DTS_DATATYPE:
                            // 如果是iOS项目并且数据模型有变化, 则把已有的数据模型都先删除, 然后再创建这个目录
                            if (this.isIosProject() && this.diffResult.datatypeChanged && isIOSDatatypeDirRemoved) {
                                logger.log('debug', {
