...
class Builder extends EventEmitter {
    ...
    buildInterfaceMock() {
        ...
        this.options.ds.interfaces.forEach((itf)=> {
            ...
            let json = mockDataWork.getParameterMockData(this.data.ds.constraints, itf.resFormat, itf.params.outputs, this.options.ds.datatypes);
            if(json.error){
                logger.log("error", {message:json.error});
            }
            
            this.output(mockDataFile, JSON.stringify(json.json, null, 4));
            // 输出 mock filter
    ...
    buildTemplateMock() {
        ...
        this.options.ds.templates.forEach((it)=> {
            ...
            if (params && params.length) {
                json = mockDataWork.getParameterMockData(this.data.ds.constraints, neiDbConst.MDL_TYP_NORMAL, params, this.options.ds.datatypes)
            }
            if(json.error){
                logger.log("error", {message:json.error});
            }
