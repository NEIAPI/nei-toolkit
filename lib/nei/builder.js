...
class Builder extends EventEmitter {
    ...
    constructor(options) {
        ...
        if(this.args.key){
            ...
            let productName;
            let condition1 = (this.config.action==="build" || diff.datatypeChanged || diff.interfaceChanged) && this.options.args.specType === 'ios';
            if(condition1 || (this.options.args.specType === 'ios' && this.options.args.overwrite)){
                ...
                if (!productName) {
                    return logger.log('error', {
                        message: `Can't find .xcodeproj file`
                    });
                }
    ...
    buildInterfaceMock() {
        ...
        this.options.ds.datatypes.forEach(dataType=>{
              mockDataWork.getMockData(this.data.ds.constraints, dataType.params,null);
        });
        this.options.ds.interfaces.forEach((itf)=> {
            ...
            mockDataWork.getMockData(this.data.ds.constraints, itf.params.outputs,this.options.ds.datatypes);
    ...
    buildDocsTree() {
        ...
        let importCustomHandleBarHelper = function(docs) {
            ...
            let sandbox = {Handlebars,neiDbConst};
    ...
    output(file, content, ignoreCheck) {
        ...
        if (file.trim() === this.config.outputRoot) {
            return logger.log('debug', {
               message: `文件名为空字符串, 不生成`
            });
        }
