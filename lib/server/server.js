...
// 监听nei配置文件变化
function watchConfigForNei(configFilePaths, server) {
          ...
          if (server) {
            ...
            configFilePaths.forEach(e => {
              try {
                ...
                _logger.error(`找不到文件${e}`);
              }
            });
            temp.routes = routesAll;

            let options = wrapOptionsForNei(temp);
...
module.exports = function (config) {
  ...
  if (options.fromNei) {
    ...
    if((typeof serverConfig === "string") && (serverConfig.constructor === String)){
      options = wrapOptionsForNei(require(serverConfig));
    }else{
      options = wrapOptionsForNei(serverConfig);
    }
  }
  let server = new Server(options);
  server.start();
  if (config.watchConfigPaths) {
    watchConfigForNei(config.watchConfigPaths, server);
  }else{
    watchConfigForNei([serverConfig],server)
  }
};
