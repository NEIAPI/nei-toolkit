...
function getModelFromServer(pagePath, callback, req) {
  ...
  if (typeof reqUrl === 'function') {
    modelServerOptions.path = pagePath;
    reqUrl = reqUrl(modelServerOptions);
  }
  ...
  require(urlParseResult.protocol.slice(0,-1)).get(options, function (res) {
    ...
    res.on('end', function () {
      ...
      try {
        ...
        console.log(`请求没有返回 json 数据：${JSON.stringify(ret)}`);
      }
      callback(json);
    });
  }).on('error', function (error) {
...
function wrapPage(index, list) {
  return function (req, res, next) {
    ...
    if (ROOT.MODEL_SERVER) {
      getModelFromServer(conf.p, function (json) {
        render(json);
      }, req);
    } else {
