...
function getFromNEISite(type, id, callback, req) {
  ...
  let host = ROOT.ONLINE_SERVER || 'https://nei.netease.com';
  let url = `${host}/api/mockdata?` + querystring.stringify(params);
...
function getModelFromServer(pagePath, callback, req) {
  ...
  let reqHeaders = {};
  // 需要判断有没有 cookie，没有的话，值是 undefined，最后发请求的时候会报错
  // 重现方法：本地删除cookie再发请求
...
module.exports = {
  ...
  setting: function (obj) {
    ...
    ROOT.ONLINE_SERVER = obj.onlineServer;
    ROOT.VIEW = obj.view;
