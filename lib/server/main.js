  ...
  init: function () {
    ...
    server.on('listening', function () {
      ...
      function displayServer(options) {
        ...
        // instantiate
        let head = ['App name', 'id', 'key', 'output', 'reload', 'launch', 'port', 'proxy-routes', 'proxy-model','date'];
        ...
        let table = new Table({
          ...
          colWidths: [10, 10, 20, 15, 10, 10, 10, 15, 15,30]
        });
        let basenameSplits = path.basename(options.project).split('.');
        // table is an Array, so you can `push`, `unshift`, `splice` and friends
        table.push(
          [!!options.name ? options.name : 'app', basenameSplits[1], basenameSplits[2], options.output, status(options.reload), status(options.launch), options.port, status(options['proxy-routes']), status(options['proxy-model']),util.getFormatTime("%s-%s-%s %s:%s:%s")]
        );
