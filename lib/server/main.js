    ...
    server.on('listening', function () {
      ...
      function displayServer(options) {
        ...
        let table = new Table({
          head: head,
          chars: chars,
          colWidths: [10, 15, 10, 10, 10, 15, 15, 15, 20]
        });
        let basenameSplits = path.basename(options.project).split('.');
        // table is an Array, so you can `push`, `unshift`, `splice` and friends
        table.push(
          [!!options.name ? options.name : 'app', options.output, status(options.reload), status(options.launch), options.port, status(options['proxy-routes']), status(options['proxy-model']), options['user-agent'], options['config-path']]
        );
