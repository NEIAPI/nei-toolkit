...
Server.prototype = {
  ...
  init: function () {
    ...
    if (options.nei && typeof options.engine === 'string') {
      ...
      // fix
      // 原来的逻辑是这样的: 如果后缀是html,那么默认会将freemarker的渲染引擎设置为html的渲染引擎.
      // 修改后的逻辑是这样的: 如果后缀是html,默认设置ejs的引擎
      if (renderMap[options.engine]) {
        if (ext === 'html') {
          engine[ext] = renderMap['ejs'];
        } else {
          engine[ext] = renderMap[options.engine];
        }
      }
