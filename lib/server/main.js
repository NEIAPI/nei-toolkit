  ...
  init: function () {
    ...
    server.on('listening', function () {
      ...
      function displayServer(options) {
        ...
        table.push(
          [!!options.name ? options.name : 'app', options.output, status(options.reload), status(options.launch), options.port, status(options['proxy-routes']), status(options['proxy-model']), options['user-agent'], options['config-path'] ? options['config-path'] : 'server.config.js']
        );
