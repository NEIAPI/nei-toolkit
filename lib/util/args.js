...
/**
 * 将 -c(或者 --config, 下同) 参数指定的数据 data 合并到 config 项里面
 * 将 data 中的参数路径转换为相对于 -c 参数指定的文件路径
 * config 中的参数（即通过命令行指定的参数）优先级高于通过 -c 指定的参数
 * @param {Object} config  - 配置标识
 * @param {Object} data - 数据信息
 * @param {String} configFilePath - 参数 -c 指定的文件路径
 */
pro._merge = (config, data, configFilePath) => {
    var convertPaths = ['p', 'project', 'o', 'output', 'tp', 'templatePath', 'tdp', 'templateDataPath'];
    var configFileDir = path.dirname(configFilePath) + '/';
    Object.keys(data).forEach((key) => {
        if (!config.hasOwnProperty(key)) {
            if (convertPaths.indexOf(key) !== -1) {
                config[key] = _path.absolute(
                    data[key], configFileDir
                );
            } else {
                config[key] = data[key];
            }
...
pro.format = function(key,data){
    ...
    // 如果有 -c 或者 -config 参数
    var config = data.c || data.config;
    if (config) {
        var configPath = _path.absolute(
            config, process.cwd()+'/'
        );
        this._merge(data, _util.file2json(configPath), configPath);
    }
    // map brief name
    var ret = {};
