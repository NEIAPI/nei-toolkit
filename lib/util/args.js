        ...
        let checkBuildConfig = () => {
            ...
            if (/^(oc)$/.test(config.lang)) {
                if (config.productName === true || !config.productName) {
                    this.log(`错误: 请指定 "productName" 的值`);
                    this.emit('exit', 1);
                }
                if (config.namePrefix === true || !config.namePrefix) {
                    this.log(`错误: 请指定 "namePrefix" 的值`);
                    this.emit('exit', 1);
                }
                if (!config.templateDataPath) {
                    this.log("错误: 请指定项目模板的数据文件的路径");
                    this.emit('exit', 1);
                }
                if (!_fs.exist(config.templateDataPath)) {
                    this.log("错误: 项目模板的数据文件不存在");
                    this.emit('exit', 1);
                }
            }
        };
        let checkMobileConfig = () => {
            if (/^(oc)$/.test(config.lang)) {
                if (config.namePrefix === true || !config.namePrefix) {
                    this.log(`错误: 请指定 "namePrefix" 的值`);
                    this.emit('exit', 1);
