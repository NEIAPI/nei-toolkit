...
class Args extends EventEmitter {
    ...
    format(key, args) {
        ...
        Object.keys(args).forEach((arg) => {
            ...
            if (!item) {
                args.templateData = args.templateData || {};
        ...
        return this.filterConfig(key, args);
    }
    ...
    filterConfig(key, config) {
        ...
        params.forEach((param) => {
            if (config.hasOwnProperty(param.q)) {
                result[param.q] = config[param.q];
            } else {
                if (!result.config.hasOwnProperty(param.q)) {
                    result.config[param.q] = config[param.q];
                }
            }
        });
        console.log(result)
