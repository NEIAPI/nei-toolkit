 ...
 * @param  {Boolean} [exitIfNotExist] - exit process if file not exist
 * @return {Object} json object
 */
exports.file2json = function (file, exitIfNotExist) {
    var ret;
    try {
        ret = require(file);
    } catch (ex) {
...
/**
 * 获取 neihost 配置信息
 * 如果没有设置过 nei 配置信息, 则直接退出程序
 */
exports.getLocalConfig = function () {
    var config = null;
    var configFilePath = '/usr/local/.neiconfig';
    var errorMsg = `请设置您所使用的NEI服务器地址: nei config set neihost [Your NEI host]。输入 nei config -h 查看更多用法`;
    try {
        config = JSON.parse(_fs.read(configFilePath).join(''));
        if (!config.hasOwnProperty('neihost')) {
 ...
 * 设置 neihost 配置信息
 */
exports.setLocalConfig = function (key, value) {
    var config = null;
    var configFilePath = '/usr/local/.neiconfig';
    try {
        config = JSON.parse(require('fs').readFileSync(configFilePath, 'utf8'));
    } catch (e) {
        config = {};
    }
    config[key] = value;
    _fs.write(configFilePath, JSON.stringify(config));
};