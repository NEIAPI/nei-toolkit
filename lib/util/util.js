 ...
 */
exports.file2json = function (file, exitIfNotExist, message) {
  var ret;
  try {
    ret = require(file);
  } catch (ex) {
...
exports.checkNodeVersion = function () {
  var compareVersion = [4, 2, 1];
  var version = process.version.substr(1).split('.');
  for (var i = 0; i < 3; i++) {
    if (parseInt(version[i]) < compareVersion[i]) {
      console.log('请将 Node 更新至 4.2.1 及以上版本，可以使用 nvm 在本地安装并管理多个 Node 版本。');
      process.exit(1);
    }
  }
};