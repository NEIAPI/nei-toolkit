...
(function () {
    ...
    function mockFunc() {
        ...
        var mockData = {
            evalExp: function (genExp, param, ds) {
                try {
            ...
            getSysDatatype: function (dataType) {
                ...
                if (!dataType.defaultValue && dataType.genExpression) {
                    return this.evalExp(dataType.genExpression, dataType, this.ds);
                }
                ...
                params.forEach(function (param) {
                    ...
                    if (param.isArray.toString() === '1') {
                        ...
                        if (param.defaultValue) {
                            try {
                                // 考虑用户输入数组的情况
                                result[param.name] = eval(param.defaultValue);
                            } catch (e) {
                                mockArr();
                            }
                        } else if (param.genExpression) {
                            result[param.name] = this.evalExp(param.genExpression, param, this.ds);
                        } else {
                            mockArr();
                        }
                    } else {
             ...
             * @param {String} genExpression - 数据模型的生成规则
             * @param {Array} traversePath - 遍历路径
             * @property {Number} traversePath.id - 路径结点id
             * @property {String} traversePath.name - 路径结点名称
             */
            getDataType: function (id, defaultValue, genExpression, traversePath) {
                ...
                if (genExpression) {
                    // 然后考虑生成规则
                    var expValue = this.evalExp(genExpression, dataType, this.ds);
                    if (expValue) {
                        return expValue;
                    }
                }
                switch (dataType.format) {
                    ...
                    case 1:
                        ...
                        if (defaultValue) {
                            ...
                            return this.getSysDatatype({
                                ...
                                genExpression: dataType.params && dataType.params[0].genExpression,
                                name: dataType.name
                            });
                        } else {
                    ...
                    case 2:
                        ...
                        var genArr = function () {
                            ...
                            var dv = param && param.defaultValue;
                            var genExpression = param && param.genExpression;
                            var v1 = this.getDataType(arrElementType, dv, genExpression, traversePath);
                            traversePath.length = pathNodeNum;
                            pathNodeNum = traversePath.length;
                            var v2 = this.getDataType(arrElementType, dv, genExpression, traversePath);
                            traversePath.length = pathNodeNum;
                            pathNodeNum = traversePath.length;
                            var v3 = this.getDataType(arrElementType, dv, genExpression, traversePath);
                            return [v1, v2, v3];
                        }.bind(this);
                    ...
                    case 5:
                        ...
                        return this.getSysParam({
                            ...
                            genExpression: genExpression || (param ? param.genExpression : ''),
                        });
                    case 6:
                        // 文件
            ...
            getParam: function (param, traversePath) {
                ...
                if (param.type <= 10003) {
                    ...
                    return this.getDataType(param.type, param.defaultValue, param.genExpression, traversePath);
                }
            },
            /**
             * 获取参数的 mock 数据
            ...
            getParams: function (format, params, ds, arrayElementCount) {
                ...
                var result = null;
                // 获取某个非 hash 数据类型的默认值
                var getDatatypeDefaultValueAndGenExp = function (id) {
                    ...
                    return {
                        ...
                        genExpression: dataType.params[0] && dataType.params[0].genExpression
                    };
                }.bind(this);
                this.ds = ds;
                switch (format) {
                    case 0:
                        result = this.getHashParams(params, []);
                        break;
                    case 1:
                        result = this.getEnumParams(params);
                        break;
                    case 2:
                        var param = params[0];
                        // 先考虑参数本身的默认值和生成规则
                        if (param.defaultValue) {
                            try {
                                // 考虑用户输入的默认值为数组的情况
                                result = eval(param.defaultValue);
                            } catch (e) {
                            ...
                            result = this.evalExp(param.genExpression, mockParam, this.ds);
                        } else {
                            // 数组, 生成三组数据
                            // 数组元素的类型从 params 中取
                            var arrElementType = param.type;
                            arrayElementCount = arrayElementCount == undefined ? 3 : arrayElementCount;
                            var genArr = function () {
                                var dvAndGenExp = getDatatypeDefaultValueAndGenExp(arrElementType);
                                var arr = [];
                                for (var i = 0; i < arrayElementCount; i++) {
                                    arr.push(this.getDataType(arrElementType, dvAndGenExp.defaultValue, dvAndGenExp.genExpression, []))
                                }
                                return arr;
                            }.bind(this);
                            if (params[0].isArray) {
                    ...
                    case 5: // 布尔
                        var param = params[0];
                        // 先考虑参数本身的默认值和生成规则
                        if (param.defaultValue || param.genExpression) {
                            result = this.getSysParam({
                                type: param.type,
                                defaultValue: param.defaultValue,
                                genExpression: param.genExpression,
                            });
                            ...
                            var dvAndGenExp = getDatatypeDefaultValueAndGenExp(arrElementType);
                            result = this.getDataType(arrElementType, dvAndGenExp.defaultValue, dvAndGenExp.genExpression, []);
                        }
                        break;
                    // case 6:
                    //     // 文件
            ...
            get: function (id, ds) {
                this.ds = ds;
                this.errorData = [];
                var json = this.getDataType(id, null, null, []);
                return {
                    json: json,
                    error: this.errorData
                };
        ...
        var publicRuleMethods = {
            ...
            repeat: function (param, ds, num, arrEleGenFunc) {
                if (param.isArray) {
                    ...
                    return mockData.getParams(2, [param], ds, num).json;
                }
            },
            /**
            ...
            loop: function (param, ds, num, arrRepeatNum) {
                num = isNaN(num) ? 3 : num <= 0 ? 3 : num;
                var genMock = function (param) {
                    var data = JSON.parse(JSON.stringify(param));
                    var loopParams = param.params.filter(function (p) {
                        return p.type === param.id;
                    });
                    param.params = param.params.filter(function (p) {
                        return p.type !== param.id;
                    });
                    var json = mockData.get(param.id, ds).json;
                    loopParams.forEach(function (p) {
                        var pointer = json;
                        var i = num - 1;
                        while (i) {
                            pointer[p.name] = mockData.get(param.id, ds).json;
                ...
                var result = null;
                if (param.isArray) {
                    var datatype = ds.find(function (p) {
                        return p.id === param.type;
                    });
                    arrRepeatNum = isNaN(arrRepeatNum) ? 3 : arrRepeatNum <= 0 ? 3 : arrRepeatNum;
                    result = [];
                    for (var i = 0; i < arrRepeatNum; i++) {
                        result.push(genMock(datatype));
                    }
                } else {
                    result = genMock(param);
                }
                return result;
            }
        };
