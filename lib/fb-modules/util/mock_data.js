    ...
    _evalExp: function (genExp, param, ds) {
      try {
        ...
        var helper = function(expr){ // 用递归可能好理解些吧
          ...
          var remain    = expr.slice(l + 1);
          let called = false;
          if(!firstPart.startsWith("Mock")){
            firstPart += '.call({param, ds}'
            called = true;
          }else{
            firstPart += '('
          }
          var r = remain.lastIndexOf(')');
          if(r === -1){
            throw new Error("语法错误,请检查生成规则");
          }
          var remainPart = remain.slice( r + 1); // ')'后面的直接加
          var subExpr = remain.slice(0, r); // 不包括')'
          var subParseResult = helper(subExpr);
          return firstPart + (subParseResult.length && called? (',' + subParseResult) : subParseResult)  + ')' + remainPart;
        };
        return eval(helper(genExp));
      } catch (e) {
