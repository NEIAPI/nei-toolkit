        ...
        var getDefaultValue = function (param) {
          ...
          if (datatype.format === 0 || datatype.format === 2) {
            try {
              ...
              console.warn(`默认值使用 JSON.parse 方法解析失败: ${e}, 相关数据模型是：${datatype.name}, 参数是：${param.name}`);
              try {
                dv = eval('(' + dv + ')');
              } catch (e) {
                console.warn(`默认值使用 eval 方法解析失败: ${e}, 相关数据模型是：${datatype.name}, 参数是：${param.name}`);
              }
            }
          }
          return dv;
