...
(function () {
  ...
  var LAST_PINYIN_UNIHAN = FIRST_PINYIN_UNIHAN + 1000;
  var NORMAL_CHINESE_CHARS = [];
  for (var i = FIRST_PINYIN_UNIHAN; i <= LAST_PINYIN_UNIHAN; i++) {
    NORMAL_CHINESE_CHARS.push(String.fromCharCode(i));
  }
  NORMAL_CHINESE_CHARS = NORMAL_CHINESE_CHARS.join('');
  ...
  var mockData = {
    _evalExp: function (genExp, param, ds) {
      try {
        genExp = genExp.replace(/NEI\.(id|str|chinese|email|url|num|bool|var|repeat|loop)\((.*?)\)/g, function ($0, $1, $2) {
